...
// Refactored to handle multiple URL formats and Vercel slash issues
...
function showUsage(help_file, headers, response) {
  ...
  var usage = `CORS Proxy Server - Usage Instructions

Method 1 - Direct URL format:
  /https://example.com
  /http://example.com

Method 2 - Full URL format (handles Vercel slash issues):
  /https:/example.com (automatically fixed to https://example.com)
  /https://example.com

Examples:
  ${headers.host}/https://api.github.com/users/octocat
  ${headers.host}/https:/google.com (auto-corrected)
  ${headers.host}/http://httpbin.org/get

The server automatically handles missing slashes in protocol URLs.
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // Enhanced URL extraction - handle both formats
    var targetUrl = req.url.slice(1); // Remove leading slash
    ...
    var location = parseURL(targetUrl);
...
function startServer(options) {
  const server = exports.createServer(options);
  server.listen(PORT, HOST, () => {
    console.log(`Enhanced CORS Proxy running on ${HOST}:${PORT}`);
    console.log('Supported formats:');
    console.log('  - /https://example.com');
    console.log('  - /https:/example.com (auto-corrected)');
    console.log('  - /http://example.com');
    console.log('  - /http:/example.com (auto-corrected)');
    console.log('');
...
startServer({
  originBlacklist: [],
  originWhitelist: [], // Empty whitelist allows all origins
  requireHeader: null,
  maxRedirects: 10
});
